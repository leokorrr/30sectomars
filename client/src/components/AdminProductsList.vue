                ...
                <tbody>
                    <tr v-for="(product, index) of products" v-bind:key="index">
                        ...
                        <td v-if=product.ingredients>
                            <div v-for="(ingredient, index) of product.ingredients" :key="index">
                                {{ingredient}}
                            </div>
                        </td>
                        <td v-if=product.weights>
                            <div v-for="(weight, index) of product.weights" :key="index">
                                {{weight}} g
                            </div>
                        </td>
                        ...
                        <td v-if=product.sizes>
                            <div v-for="(size, index) of product.sizes" :key="index">
                                {{size}} cm
                            </div>
                        </td>
                        <td v-if=product.prices>
                            <div isMultiPrice = true v-for="(price, index) of product.prices" :key="index">
                                {{price}}$
                            </div>
                        </td>
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body" v-if="updateModal !== null">
                        <form action="">
                            ...
                            <div class="form-group">
                                ...
                                <input class="form-control" id="description" placeholder="Enter description" v-model="updDescription">
                            </div>
                            <div class="form-group" v-if=ingredients>
                                <label for="ingredients">Ingredients</label>
                                <input class="form-control" id="ingredients" placeholder="Enter ingredients" v-model="updIngredients">
                            </div>
                            <div class="form-group" v-if=isMultiWeight>
                                <label for="weights">Weights</label>
                                <input class="form-control" id="weights" placeholder="Enter weights" v-model="updWeights">
                            </div>
                            <div class="form-group" v-if=!isMultiWeight>
                                <label for="weight">Weight</label>
                                <input class="form-control" id="weight" placeholder="Enter weight" v-model="updWeight">
                            </div>
                            ...
                            <div class="form-group" v-if=isMultiPrice>
                                ...
                                <input class="form-control" id="prices" placeholder="Enter prices" v-model="updPrices">
                            </div>
                            <div class="form-group" v-if=!isMultiPrice>
                            ...
                            <div class="form-group" v-if=quantity>
                                ...
                                <input class="form-control" id="quantity" placeholder="Enter hot" v-model="updQuantity">
                            </div>
                            <div class="form-group" v-if=substanceAmount>
                                <label for="substanceAmount">Substance amount</label>
                                <input class="form-control" id="substanceAmount" placeholder="Enter substance amount" v-model="updSubstanceAmount">
                            </div>
                            <div class="form-group" v-if=origin>
                                <label for="origin">Origin</label>
                                <input class="form-control" id="origin" placeholder="Enter origin" v-model="updOrigin">
                            </div>
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-primary close" data-dismiss="modal" aria-label="Close" @click="updateProduct">Save changes</button>
                    </div>
                </div>
            </div>
        </div>
...
export default {
  ...
  methods: {
    getProduct() {
        axios.get(`http://192.168.0.164:5000/api/${this.productName}`)
            .then(res => {
    ...
    deleteProduct(e) {
        axios.post(`http://192.168.0.164:5000/api/${this.productName}-delete`,{
    ...
    onSearch(keyword) {
        ...
        axios.post(`http://192.168.0.164:5000/api/${this.productName}-search`, {
